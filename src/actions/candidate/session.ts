...
export async function initializeSessionAction(interviewId: string): Promise<ActionResult<{ interview: unknown }>> {
  try {
...
export async function streamAiTurnAction(interviewId: string, candidateResponse?: string) {
  const session = await getSession()
  ...
  const previousTopics = interview.topics.filter(t => t.topicIndex < activeTopic.topicIndex && t.status === 'CLOSED')
  ...
  ;(async () => {
    try {
      const planConfigSnapshot = interview.planConfigSnapshot as Record<string, unknown> | null
      ...
      if (isOpening) {
        aiResult = await generateOpeningQuestion(candidateProfile, plannedTopic, previousTopics)
      } else {
        const followUpCount = activeTopic.turns.filter(t => t.role === 'AI' && t.turnType === 'FOLLOWUP').length
        const planConfigSnapshot = interview.planConfigSnapshot as Record<string, unknown> | null
        const maxFollowUps = (planConfigSnapshot?.maxFollowUpsPerTopic as number) || 2
        const maxFollowUpsReached = followUpCount >= maxFollowUps

        aiResult = await generateFollowUp(candidateProfile, plannedTopic, previousTopics, activeTopic.turns, maxFollowUpsReached)
      }
      ...
      if (moveToNext) {
        ...
        const nextTopic = interview.topics.find(t => t.topicIndex === activeTopic.topicIndex + 1)
        if (nextTopic) {
          await prisma.interviewTopic.update({
      ...
      console.error('🔥 [streamAiTurnAction stream error] FATAL ERROR IN BACKGROUND AI STREAM:', err)
